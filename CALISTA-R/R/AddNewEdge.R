AddNewEdge<-function(h,node,colorMARK2,x_center,y_center,z_center,
                     ClusterGroup2,expected_clusters,normed_pseudotime,score3,legendInfo,az,al){
  gg_data=data.frame()
  label_color=c()
  label_names=c()
  ...
  h=add_edges(h,c(node[1],node[2]),weight=node[3])
  V(h)$size=1
