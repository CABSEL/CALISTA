CALISTA_path<-function(Results,INPUTS){
  ...
  while(add_paths){
    ...
    writeLines('Please 1 to add another path, 0 otherwise:')
  ...
  for(i in 1:n_paths){
    ...
    radius_width=round(max(length(cells_in_path)/20,10))
    ...
    if(hclustering){
      ...
      hv=heatmap.2(transition_expression,
                 ...
                 ColSideColors = ColumnLabelsColorValue$Colors, 
                 main = paste("Hierarchical clustering for path num:",i))
      col_anno_names=data.frame(samples=ColumnLabelsColorValue$Labels)
      col_anno_colors=ColumnLabelsColorValue$Colors
      names(col_anno_colors)=col_anno_names
      col_anno_colors=list(samples=col_anno_colors)
      pheatmap::pheatmap(transition_expression,
                         cluster_cols = FALSE,
      ...
      H1ESC_DEC_bulk_topGenes=rownames(topTable(H1ESC_DEC_bulk_vf,coef = 2,number = 200))
      H1ESC_DEC_bulk_anno_names=data.frame(samples=H1ESC_DEC_bulk_labels)
      H1ESC_DEC_bulk_anno_colors=list(samples=c(H1_="red",DEC="blue"))
      pheatmap(log10(H1ESC_DEC_bulk[H1ESC_DEC_bulk_topGenes,]+1.01),
               cluster_rows = FALSE,cluster_cols = FALSE,
               annotation_col = H1ESC_DEC_bulk_anno_names,
               annotation_colors = H1ESC_DEC_bulk_anno_colors,
