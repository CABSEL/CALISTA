function ydata = tsne2(X, labels, no_dims, initial_dims, perplexity)
...
% (C) Laurens van der Maaten, Delft University of Technology
    ...
    if ~initial_solution
        ...
        if size(X, 2) < size(X, 1)
            ...
            C = (1 / size(X, 1)) * (X * X'); %Covariance matrix
        end
        [M, lambda] = eig(C);
    ...
    % Run t-SNE
    if initial_solution
        ydata = tsne_p2(P, labels, ydata);
    else
        ydata = tsne_p2(P, labels, no_dims);
    end