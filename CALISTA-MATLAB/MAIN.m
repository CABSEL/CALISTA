...
INPUTS.data_type=4; % Single-cell RT-qPCR CT data
INPUTS.format_data=2; % Rows= cells and Columns= genes with time/stage info in the last column
INPUTS.data_selection=[]; % Include data from all time points
INPUTS.perczeros_genes=90; % Remove genes with > 100% of zeros 
INPUTS.perczeros_cells=100; % Remove cells with 100% of zeros
INPUTS.cells_2_cut=0; % No manual removal of cells
INPUTS.perc_top_genes=10; % Retain only top X the most variable genes with X=min(200, INPUTS.perc_top_genes * num of cells/100, num of genes)
